...
Плеер работает автономно на клиентском устройстве, обращаясь к API музыкальных платформ для авторизации, поиска и стриминга. Данные о локальной медиатеке, кэше и плейлистах сохраняются локально.

```mermaid
graph TD
    subgraph Client ["Аудиоплеер Quark (Flutter)"]
        UI["Интерфейс пользователя (Glassmorphism)"]
        Engine["Аудио-движок (Just Audio / Media Kit / SMTC / MPRIS)"]
        Storage["Локальное хранилище (Drift SQLite / Hive)"]
    end

    subgraph External ["Внешние музыкальные сервисы"]
    ...
    UI -->|Авторизация и стриминг| YM
    UI -->|Импорт плейлистов| SP
    Engine -->|Проигрывание потоков| YT
    Engine -->|Проигрывание потоков| SC
    YM -->|Экспорт FLAC с метаданными| Storage
```
